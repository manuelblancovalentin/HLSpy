...
VerilogBlock Parser::__parse__(std::vector <VerilogBlock>& module_definitions,
                               ...
                               int nlines, int line_offset,
                               std::string TAB, std::string NAME) {

    // This is not the nicest thing, but it works for now.
    // So I'm gonna open two streams here: one will be a filestream to read from FILENAME
    // and the other will be a string stream to read from TEXT.
    // We are only gonna read, in practice, from one of them, but as it looks like
    // this method is easier to implement than to try to convert between types of
    // streams, etc., this is what we are gonna do. Yes, I admit it, this is not
    // beautiful coding, I'm sorry. It's functional tho.
    std::istrstream stream_str(TEXT.c_str());
    ...
    int delta = 100; //(int) ((double) nlines * 0.01);
    ...
    while (end_line < nlines) //!stream.eof()
    {
        // read next line
        getline(stream, line_file);
        getline(stream_str, line_str);
        ...
        if (stringStartsWith(line, "//")) {
            ...
            if ((nbegins > nends) || (nbegins == 0 && nends == 0)) {
                ...
                throw std::invalid_argument("Something went wrong while parsing block comment (we never found closing brackets). Raising error.");
            }
            ...
            int nbegins = string_count(line, __VLOG_MODULE_REGEX__);
            int nends = string_count(line, __VLOG_ENDMODULE_REGEX__);
            ...
            if ((nbegins > nends)  || (nbegins == 0 && nends == 0)) {
                ...
                // Now let's create new Module
                // First thing that we have to do is to remove the "module <NAME>;" and the "endmodule" from
                // the text, otherwise we will enter an eternal loop that will never end.
                std::size_t semicolon_pos = line.find(';');
                std::string module_content = "";

                if (semicolon_pos == std::string::npos) {
                    // Something went wrong, throwing exception
                    throw std::invalid_argument("Something went wrong while parsing the module. Raising error.");
                }

                // Divide semicolon
                module_content = line.substr(semicolon_pos + 1);
                line = line.substr(0, semicolon_pos + 1);

                // Trim
                ltrim(module_content);
                rtrim(line);

                // Now we also need to get rid of the "endmodule" part.
                std::size_t endmodule_pos = module_content.rfind("endmodule");
                ...
                //
                module_content = module_content.substr(0, endmodule_pos);
                ...
                message = string_format("Module %s found from line %d to %d\n", module_name.c_str(),
                                        start_line + line_offset, end_line + line_offset);
                pbar.update(end_line, nlines - 1, message, TAB);


                // Init parser object
                Parser m("", module_content, progressBar(),
                         num_lines = end_line - start_line + 1,
                         line_offset = line_offset + start_line - 1,
                         TAB = TAB + "\t", NAME = module_name);
                // Parse
                VerilogBlock vlogtmpmod = m.parse(module_definitions, sources, lib);
        ...
        // Update progress bar
        if (((end_line % delta == 0 || end_line >= (nlines - 1)) || force_print) && (message != ""))
            pbar.update(end_line, nlines - 1, message, TAB);
